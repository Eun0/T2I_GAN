...
from model.df_gan_dec import DF_GEN, DF_DISC
...
def parse_args():
    ...
    parser.add_argument('--cfg',type=str,default='cfg/df_gan/df_gan_sbert_ft_img_match_global_sent_withGD_dec.yml')
...
def train(args, cfg, train_set, train_loader, test_loader, state_epoch, text_encoder, netG, netD, optimizerG, optimizerD, 
    ...
    for epoch in range(state_epoch+1, cfg.TRAIN.MAX_EPOCH + 1):
        ...
        for step, data in enumerate(train_loader):
            ...
            real_features, rec_features = netD(imgs)
            ...
            fake_features, _ = netD(fake.detach())
            ...
            dec_loss = 0.
            if cfg.TRAIN.ENCODER_LOSS.DEC:
                assert cfg.DISC.IS_REC
                rec_reals = netD.D_REC(rec_features)
                dec_loss = torch.mean(torch.abs(rec_reals - imgs))
            ...
            errD = errD_real + (mis_loss * cfg.TRAIN.SMOOTH.MISMATCH) + enc_loss + dec_loss
            ...
            if i % cfg.TRAIN.N_CRITIC == 0:
                ...
                if cfg.TRAIN.ENCODER_LOSS.DISC != '':
                    real_features, _ = netD(imgs).detach()
        ...
        if args.log_type == 'wdb':
            ...
            log_dict.update({'dec_loss':dec_loss.item()}) if cfg.TRAIN.ENCODER_LOSS.DEC else None
            wandb.log(log_dict)
        else:
            ...
            writer.add_scalar('dec_loss', dec_loss.item(), epoch) if cfg.TRAIN.ENCODER_LOSS.DEC else None
...
if __name__ == '__main__':
    ...
    #cfg.TRAIN.ENCODER_LOSS.DEC = True
    #cfg.DISC.IS_REC = True
    ...
    netD = d_arch(cfg, cond_dim = cond_dim, is_rec = cfg.DISC.IS_REC, is_disc = True).cuda()
