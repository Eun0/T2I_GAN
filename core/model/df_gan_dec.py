...
class DF_GEN(nn.Module):
    def __init__(self, cfg, cond_dim, **kwargs):
        super(DF_GEN, self).__init__()
        ...
        if cfg.GEN.PROJ_TEXT:
            self.proj_sent = nn.Linear(cond_dim, nef)
        else:
...
class DF_DISC(nn.Module):
    def __init__(self, cfg, cond_dim, is_rec=False, **kwargs):
        super(DF_DISC, self).__init__()
        ndf = cfg.DISC.NCH
        ...
        self.COND_DNET = D_GET_LOGITS(cfg, cond_dim = cond_dim, ndf=ndf, spec_norm = spec_norm)
        
        if is_rec:
            self.D_REC = DECODER(ndf = ndf)
...
class D_GET_LOGITS(nn.Module):
    def __init__(self, cfg, cond_dim,  ndf, spec_norm = False):
        ...
        if self.img_match:
            ...
            self.proj_match = linear(cond_dim, ndf * 16, spec_norm=spec_norm) # sent    
        else:
    ...
    def forward(self, x, sent_embs, **kwargs):
        ...
        out_img = F.avg_pool2d(x, kernel_size = 4)
